...
def plot_scattering(df, samples, colors, f_out, show_legend=True):
	""" Plot the scattering for each capillary """
	# Select columns
	cols_sel = [colname for colname in df.columns if colname.startswith('Scattering')]
	...
	ax.set_ylabel('Scattering (counts)', fontsize=24)
	plt.savefig(f_out + '_scattering.png', transparent=True, bbox_inches='tight')
	plt.close()
...
def main():
	...
	### Create plots
	plot_radius(df, samples, colors, f_out, show_legend)
	plot_scattering(df, samples, colors, f_out, show_legend)
